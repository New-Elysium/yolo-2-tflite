...
from core.weights_loader import load_weights_from_cfg, load_weights_legacy
...
def save_tf():
  if FLAGS.generic and FLAGS.cfg:
    # Generic model building from CFG
    model = build_model_from_cfg(FLAGS.cfg)
    model = load_weights_from_cfg(model, FLAGS.cfg, FLAGS.weights)
    
    # For generic models, we need to handle post-processing differently
    # This is a simplified approach - may need customization per model
    pred = model.output
    if isinstance(pred, list):
      # Multiple YOLO outputs - concatenate for processing
      pred = tf.concat(pred, axis=-1)
    
  else:
    # Legacy model building
    STRIDES, ANCHORS, NUM_CLASS, XYSCALE = utils.load_config(FLAGS)
    
    input_layer = keras.layers.Input([FLAGS.input_size, FLAGS.input_size, 3])
    ...
    model = keras.Model(input_layer, pred)
