...
def cspdarknet53(input_data):
    ...
    # SPP
    maxpool_13 = tf.keras.layers.MaxPool2D(pool_size=13, strides=1, padding='same')(input_data)
    maxpool_9 = tf.keras.layers.MaxPool2D(pool_size=9, strides=1, padding='same')(input_data)
    maxpool_5 = tf.keras.layers.MaxPool2D(pool_size=5, strides=1, padding='same')(input_data)
    input_data = tf.keras.layers.Concatenate(axis=-1)([maxpool_13, maxpool_9, maxpool_5, input_data])
...
def cspdarknet53_tiny(input_data):
    ...
    route_1 = input_data
    input_data = common.convolutional(input_data, (3, 3, 32, 32))
    input_data = tf.keras.layers.Concatenate(axis=-1)([input_data, route_1])
    input_data = common.convolutional(input_data, (1, 1, 32, 64))
    input_data = tf.keras.layers.Concatenate(axis=-1)([route, input_data])
